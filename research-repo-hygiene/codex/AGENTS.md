# research-repo-hygiene — project structure and experiment hygiene (Codex version)

**Use this when setting up or maintaining the structure of a research or
experiment repository** — "整理项目结构", "优化文档", "tidy up the repo", "写个
docs 路由" — when a scratch or outputs directory has accumulated, when an
investigation is being closed out, or when a session cannot tell what has
already been tried.

It covers what `docs/` must contain, which pages are living, when an experiment
earns its own branch, and what to archive versus delete when a direction is
abandoned.
